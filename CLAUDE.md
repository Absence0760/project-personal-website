...
Content pages, each a hand-written Svelte component under `src/routes/`: **Home** (`src/routes/+page.svelte` — a standalone landing composition that opts out of the sidebar shell; see `docs/design-system.md` → "Landing page"), **Services** (`src/routes/services/`), **Capability Statement** (`src/routes/capabilities/` — the federal one-pager; UEI/CAGE rows are added to its `<dl class="capability-data">` block once SAM.gov registration completes, see `docs/sam-gov-checklist.md`), **CV** (`src/routes/cv/`), plus the legal pages. Every page except Home shares the `src/routes/+layout.svelte` shell (sidebar + footer); marketing/legal pages use the `.prose` layout.
...
- **Routes/components:** `src/routes/**/+page.svelte` (one per page) + `src/lib/` (`Sidebar`/`Footer` for the prose shell; `SiteHeader`/`HeroRibbon`/`WorkRow`/`WorkThumb`/`LandingFooter`/`Icon`/`Monogram` + the `reveal` action for the landing page; `site.ts` metadata). No Markdown/CMS — pages are hand-authored Svelte.
- **Styling:** a single global stylesheet `src/app.css` (design tokens + light/dark via `prefers-color-scheme`), imported once in the layout. The landing page adds a second, namespaced `--l-*` token layer under the `LANDING PAGE` banner at the foot of the file.
