...
- **Routes/components:** `src/routes/**/+page.svelte` (one per page) + `src/lib/` (shared `Sidebar`/`Footer` components and `site.ts` metadata). No Markdown/CMS — pages are hand-authored Svelte.
