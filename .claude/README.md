...
Slash commands and agents tuned for this repo — a SvelteKit personal website at
...
- **`test-gap-checker.md`** — the Vitest suite (`pnpm test`) plus
  `pnpm build` cover most regressions; this agent guards the legal-page
  cross-references and the manual-walkthrough evidence a change should ship with.
- **`ui-polisher.md`** — typography / layout polish targeted at the Svelte
  routes and components under `src/`.
...
- **`compliance-auditor.md`** — invoked by the legal-flavoured audit
  commands. Knows the legal-page commitments in `src/routes/` plus the
...
- **`polish-ui.md`** — typography / layout polish on a Svelte page or component.
...
- The agents and commands are tuned for *this site*. If they start
  emitting findings about Next.js / Hono / SOPS / Lambda / Sanity / a
