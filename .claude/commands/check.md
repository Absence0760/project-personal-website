...
- `test-gap-checker` — prompt: "Audit the working diff for missing verification (pnpm build, pnpm check, pnpm test, manual walkthrough notes, legal-page cross-reference check) per the root CLAUDE.md verification rule. Output the format from your spec. Note: this repo has a Vitest suite — flag missing test coverage where the change adds testable logic, alongside the build/walkthrough/cross-reference gaps."
