...
Project-curated slash commands for running periodic security, dependency, and compliance audits on this repo — a SvelteKit static site at `jaredhoward.com`, deployed to GitHub Pages, that doubles as a Stripe business URL.
...
| [/audit/xss](xss.md) | Svelte `{@html}` usage, client code that interpolates URL fragments / query params into the DOM, raw `<script>` / `<iframe>` in component markup |

### Health

| Command | What it checks |
|---|---|
| [/audit/deps](deps.md) | GitHub Actions pinning, Dependabot coverage, action-version drift, and the npm dependency tree (SvelteKit / Vite / Svelte + Vitest) plus its lockfile |
...
| Command | What it checks |
|---|---|
| [/audit/cookie-consent](cookie-consent.md) | Any third-party SDK / fetch / font / pixel fires on page load — should be empty per `src/routes/privacy/+page.svelte` §4 + §8 |
...
| [/audit/data-export-completeness](data-export-completeness.md) | Posture confirmation: nothing to export — confirm `src/routes/privacy/+page.svelte` still says so |
| [/audit/account-deletion-completeness](account-deletion-completeness.md) | Posture confirmation: no accounts — confirm `src/routes/privacy/+page.svelte` still says so |
| [/audit/accessibility](accessibility.md) | WCAG 2.2 AA on the deployed bundle — heading hierarchy, alt text, keyboard focus order, contrast |
...
- Cross-references: findings tie back to `CLAUDE.md` rules or `src/routes/privacy/+page.svelte` policy commitments whenever they map there.
...
- **After adding any third-party script / font / pixel / iframe to a component** — `/audit/cookie-consent` + `/audit/third-party-data-flows` (and update `src/routes/privacy/+page.svelte` §4 in the same change).
